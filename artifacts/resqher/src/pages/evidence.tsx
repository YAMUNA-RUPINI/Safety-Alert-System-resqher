...
function VideoPlayer({ idbPath, label }: { idbPath: string; label: string }) {
  const [objectUrl, setObjectUrl] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(false);

  const key = idbPath.replace("indexeddb://", "");
  ...
  if (objectUrl) {
    return (
      <div className="space-y-2">
        <p className="text-xs text-[#a78bca] font-semibold uppercase tracking-widest">{label}</p>
        <video
          src={objectUrl}
          controls
          className="w-full rounded-xl border border-[#2d1f4e] bg-black max-h-48"
        />
        <a
          href={objectUrl}
          download={`${key}`}
          className="flex items-center gap-1.5 bg-[#0d0a1a] border border-[#2d1f4e] hover:border-[#9b1d3a] text-[#a78bca] hover:text-white text-xs font-semibold px-3 py-2 rounded-lg transition-all w-fit"
        >
  ...
  if (error) {
    return (
      <div className="space-y-1">
        <p className="text-xs text-[#a78bca] font-semibold uppercase tracking-widest">{label}</p>
        <p className="text-xs text-red-400 italic">Video not available on this device</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1">
      ...
      >
        {loading ? (
          <>
            <span className="w-3 h-3 border border-[#a78bca] border-t-transparent rounded-full animate-spin" />
            Loading…
          </>
        ) : (
          <>
          ...
          <div className="space-y-4">
            {emergencies.map((item) => (
              <div
                ...
                className="bg-[#16102b] rounded-2xl border border-[#2d1f4e] p-5 space-y-4"
              >
                ...
                {(item.frontVideoLocalPath || item.backVideoLocalPath) ? (
                  <div className="space-y-4">
                    <p className="text-xs text-[#6b5e8a] uppercase tracking-widest border-t border-[#2d1f4e] pt-3">
                      Recorded Videos
                    </p>
                    <div className="grid grid-cols-1 gap-4">
                      {item.frontVideoLocalPath && item.frontVideoLocalPath.startsWith("indexeddb://") && (
                        <VideoPlayer idbPath={item.frontVideoLocalPath} label="Front Camera" />
                      )}
                      {item.backVideoLocalPath && item.backVideoLocalPath.startsWith("indexeddb://") && (
                        <VideoPlayer idbPath={item.backVideoLocalPath} label="Back Camera" />
                      )}
                    </div>
                  </div>
                ) : (
                  <p className="text-xs text-[#4a3d6b] italic border-t border-[#2d1f4e] pt-3">
                    {t("noVideoSaved")}
                  </p>
                )}
              </div>
